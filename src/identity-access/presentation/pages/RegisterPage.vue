...
const hasSuccessState = computed(
  () => identityAccessStore.registerStatus.status === "success",
);
...
function validateForm() {
  ...
  validation.firstName = !form.firstName.trim()
  ...
  if (!form.password) validation.password = t("auth.passwordRequired");
    ...
    validation.password = t("auth.passwordMinLength");
  else validation.password = "";

  if (!form.confirmPassword)
    validation.confirmPassword = t("auth.confirmPasswordRequired");
...
<template>
  <AuthLayout mode="register">
    <section
      ...
      :class="{
        'bt-auth-error': hasErrorState,
        'bt-auth-success': hasSuccessState,
      }"
    >
      <header>
      ...
      <Message
        v-if="hasErrorState && !hasSuccessState"
        severity="error"
        class="bt-auth-message"
      >
      ...
      >
        ...
        <p v-if="validation.role" class="bt-auth-helper">
          {{ validation.role }}
        </p>
        ...
        <div class="bt-auth-grid">
          <div class="bt-auth-grid-field">
            ...
            />
            <p v-if="validation.firstName" class="bt-auth-helper">
          ...
          <div class="bt-auth-grid-field">
            ...
            />
            <p v-if="validation.lastName" class="bt-auth-helper">
              {{ validation.lastName }}
            </p>
          </div>
        </div>
        ...
        <div class="bt-auth-field">
          ...
          />
          <p v-if="validation.email" class="bt-auth-helper">
            {{ validation.email }}
          </p>
        </div>
        ...
        <div class="bt-auth-grid">
          <div class="bt-auth-grid-field bt-auth-password">
            ...
            />
            <p v-if="validation.password" class="bt-auth-helper">
              {{ validation.password }}
            </p>
          </div>
        ...
        <div class="bt-auth-actions">
          <Button
            ...
            type="submit"
          />
          <RouterLink class="bt-auth-footer-link" to="/login">{{
            t("auth.goToLogin")
          }}</RouterLink>
        </div>
      </form>
